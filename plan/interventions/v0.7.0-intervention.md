...
**Started:** 2025-12-26
**Status:** ✅ QA PASSED - Ready for release
...
| Issue | Title | Status |
|-------|-------|--------|
| #33 | Expected Progress Indicator | ✅ QA Passed |
| #45 | Bar corner radius default to 10 | ✅ QA Passed |
| #43 | Remove 'Disable all Editing' option | ✅ QA Passed |
| #42 | Weekend highlighting off by default | ✅ QA Passed |
| #38 | Float dependency strings normalization | ✅ QA Passed |
| #37 | Pandas deprecation warning | ✅ Code correct (no param used) |
...
### What's Done
- [x] webapp.json: barCornerRadius default 3→10, highlightWeekends default true→false
...
### What's Pending
- [x] QA Round 5: All issues verified - PASSED
...
- `54.8` → `54_x2e_8` (period = hex 2e)
- `task 1` → `task_x20_1` (space = hex 20)

This is:
- **Deterministic**: same input → same output
- **Collision-free**: `54.8` → `54_x2e_8` vs `54_8` stays `54_8`
...
**Findings:**
1. ✅ CSS selector error gone
2. ✅ Expected progress markers visible in Week view
3. ✅ Tooltips show original IDs and dependencies
4. ❌ Month view: Markers positioned ~1 month late (bars also ~1 month late)

**Root Cause:** Frappe Gantt library bug in `diff()` function - see Bug #6 in `plan/frappe-gantt-upstream-bugs.md`

---

## Fix #4: Month view diff() bug (frappe-gantt patch)

**Problem:** In Month view, task bars positioned ~1 month late. A task starting Oct 1 appeared at Nov position.

**Root Cause:** The `diff()` function uses `o%30/30` for fractional month calculation:
- `o` = total days difference (meaningless for position)
- `o % 30` gives random value based on total span, not day-of-month

**Fix:** Changed `o%30/30` to `(n.getDate()-1)/30`:
```javascript
// BEFORE (buggy):
...
Now Oct 1 → fractional 0.0 (start of month), Oct 15 → fractional 0.467 (mid-month).

---

## QA Round 5 Instructions
...
6. Verify:
   - Task bars align with correct month columns?
   - Expected progress markers positioned correctly?
   - Hover highlight shows correct months?
   - Compare: Does Oct task bar start at Oct column (not Nov)?
7. Test Week view still works correctly
```
...
- Task bars align with correct month columns
- Expected progress markers positioned correctly
- Month view positioning bug fixed
...
| `resource/frappe-gantt.umd.js` | Month view diff() bug fix |
...
3. Awaiting QA Round 5 results
4. Month view diff() bug was in frappe-gantt library, patched in `resource/frappe-gantt.umd.js`
5. Key function: `addExpectedProgressMarkers()` in app.js (~line 847)
6. Bug #6 documented in `plan/frappe-gantt-upstream-bugs.md`
