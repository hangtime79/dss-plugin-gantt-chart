...
**Started:** 2025-12-23
**Status:** ⚠️ PARTIAL SUCCESS - Ready for branch exit
...
- View state (mode + scroll) preserved across config changes
- `renderInProgress` flag prevents overlapping async renders

**Issue #11 (Sticky Header):** ⚠️ PARTIALLY RESOLVED
- Sticky works when content fills viewport (edge-to-edge)
- Sticky janky when content narrower than viewport
- Known limitation documented in GitHub issue #21

### Key Files
| File | Purpose |
|------|---------|
| `webapps/gantt-chart/app.js` | Main frontend logic - debounce + sticky implementation |
| `resource/webapp/style.css` | CSS (sticky header CSS at line 315) |
| `plugin.json` | Version 0.4.2 |

### What Works
- View state (mode + scroll) preserved on config change
- `renderInProgress` flag prevents overlapping async renders
- Debouncing works correctly (one render per final config)
- Sticky header works when chart content fills viewport width

### Known Limitations
1. **Sticky janky with narrow content** - When SVG doesn't fill viewport (e.g., Year view with short date range) - tracked in #21
2. **Hour view slow** - inherent (1.4M px SVG) - not fixable without major refactor

### Root Cause Analysis (from console test)
Console test **DISPROVED** the duplicate config message hypothesis:
- No config messages after initial render (waited 6+ minutes)
- Only "keep alive" heartbeats every 10 seconds (not config updates)
- Debouncing is working correctly

**Actual root cause:** The `lastStickyHeader` optimization in `setupStickyHeader()` skips re-initialization when the header element reference is unchanged. Year view (narrow content, "not edge to edge") corrupts the sticky state without creating a new header element, so the state never recovers.

### Fix (Attempt 5 - Actual)
Remove the optimization and always fully re-initialize sticky header:
```javascript
function setupStickyHeader() {
    // ... container/header checks ...

    // ALWAYS clean up previous state (fixes Year view corruption)
    if (stickyScrollHandler) {
        container.removeEventListener('scroll', stickyScrollHandler);
        stickyScrollHandler = null;
    }

    // Reset transform to prevent stale state
...
Also remove the `lastStickyHeader` variable since it's no longer needed.
...
**Status:** ❌ HYPOTHESIS DISPROVED - Not implementing config hash

**Original Hypothesis:**
Dataiku sends config messages periodically (not just on user changes).

**Console Test Result:**
User ran the console test and waited 6+ minutes. Results:
- No config messages after initial render
- Only "keep alive" heartbeats every 10 seconds (from Dataiku framework, not config)
- Debouncing is working correctly - only final config triggers render

**Conclusion:** Config hash comparison is NOT needed.

---

### Attempt 5 (ACTUAL): Fix Year View State Corruption
**Status:** 🔄 IMPLEMENTING

**Discovery:**
User discovered that sticky works perfectly UNTIL Year view is opened:
- Month/Week/Day/Hour views: Sticky works perfectly
- Switch to Year view: Sticky becomes janky
- Switch back to other views: Jankiness PERSISTS

Year view is "not edge to edge" (narrow content). This somehow corrupts the sticky state.

**Root Cause:**
The `lastStickyHeader` optimization skips re-initialization when header element reference is unchanged:
```javascript
if (header === lastStickyHeader && stickyScrollHandler) {
    console.log('Sticky header already set up for this element');
    return;  // SKIPS re-init, state stays corrupted
}
```

Year view corrupts state without creating a new header element, so recovery never happens.

**Fix:**
1. Remove `lastStickyHeader` variable entirely
2. Always clean up previous scroll handler
3. Always reset transform before applying new one
4. Use `translate3d` for guaranteed GPU acceleration
...
function setupStickyHeader() {
    ...
    // Create scroll handler with GPU-accelerated transform
    stickyScrollHandler = function() {
        header.style.transform = `translate3d(0, ${container.scrollTop}px, 0)`;
    };
...
**QA Result:** ❌ PARTIAL - Removing optimization didn't fix narrow content jank

---

### Attempt 5b: Force Header Width for Narrow Content
**Status:** 🔄 IMPLEMENTING

**Discovery from QA Round 4:**
- When content fills container (edge to edge): Sticky works fine
- When content is narrower than container (e.g., Year view with short date range): Still janky
- The `lastStickyHeader` optimization removal didn't address root cause

**Root Cause (refined):**
The jankiness occurs when SVG content doesn't fill the container width. The header is narrower than the viewport, causing browser rendering/paint issues during scroll transform.

**Fix:**
Force header to span full container width regardless of content width:
```javascript
...
**QA Result:** ❌ FAILED - minWidth did not fix jankiness

**Outcome:**
- User tested with constrained Year view (narrow content)
- Sticky header still janky when content doesn't fill viewport
- Issue is deeper than header width - likely browser paint/composite behavior

**Documented as Known Limitation:**
- Created GitHub issue #21 for tracking
- Proposed future fix: Calculate minimum date range to guarantee edge-to-edge content
- Workaround: Use date ranges that span enough time to fill viewport in all views
