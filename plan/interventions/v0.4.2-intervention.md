...
**Started:** 2025-12-23
**Status:** QA Round 5 Pending - Awaiting console test results
...
### Current State
- 4 implementation attempts completed, all partially failed
- Hypothesis identified: Dataiku sends periodic config "heartbeat" messages
- User is running console test to verify hypothesis
- If confirmed, implement config hash comparison (Attempt 5)
...
- Sticky header works AFTER 90-210 seconds

### What's Broken
1. **Sticky jankiness first 90-210 sec** - likely caused by Dataiku sending repeated config messages
2. **Unexplained auto-refreshes** - same root cause
3. **Hour view slow** - inherent (1.4M px SVG) - not fixable without major refactor

### Proposed Fix (Attempt 5)
Add config hash comparison to skip duplicate updates:
```javascript
...
function hashConfig(config) {
    return JSON.stringify({
        ...
        progressColumn: config.progressColumn,
        dependencyColumn: config.dependencyColumn,
        ...
        chartStartDate: config.chartStartDate,
        chartEndDate: config.chartEndDate
    });
}

// In message handler, BEFORE debounce:
const newHash = hashConfig(webAppConfig);
if (newHash === lastConfigHash && ganttInstance) {
...
window.addEventListener('message', function(e) {
    if (e.data && typeof e.data === 'string') {
        try {
            ...
            if (d.webAppConfig) {
                msgCount++;
                const configStr = JSON.stringify(d.webAppConfig);
                const changed = configStr !== lastConfig;
                console.log(`MSG #${msgCount} @ ${new Date().toISOString().slice(11,19)} | Changed: ${changed}`);
...
### If User Confirms Hypothesis (Changed: false messages seen)
1. Implement the hashConfig function and comparison
2. Add it to message handler BEFORE the debounce logic
3. Commit and test

### If User Denies Hypothesis (No duplicate messages)
1. Look for other sources of re-renders
2. Consider adding more console.log to trace render triggers
3. May need to investigate Dataiku framework behavior
