...
- [x] Implement Month mode responsive formatting (full → 3-letter → 1-letter)
...
- [x] Implement Year mode responsive formatting (YYYY → YY)
...
Container structure:
```
#gantt-container          (overflow: auto, height: 100%) ← USER SCROLLS HERE
  └── .gantt-container    (overflow: auto, height: var(--gv-grid-height)) ← Frappe's
        ├── .grid-header  (position: sticky, top: 0) ← RELATIVE TO WRONG CONTAINER
        └── svg.gantt
```
...
- [x] If user start date < calculated start → use calculated start (don't expand left)
- [x] If user end date > calculated end → use calculated end (don't expand right)
...
|---|---------|--------|-------|
| 1 | Fix Header Text Collisions (CSS) | ✅ Complete | CSS overflow handling in style.css |
| 1b | Responsive Header Abbreviations | ✅ Fixed | Week/Month/Year all working (commit 6cd755f) |
| 2 | Sticky Header Fix (CSS-only) | ❌ Not Working | Nested scroll container issue |
| 2b | Sticky Header Fix (Option A) | 🔄 Testing | overflow-y: visible applied, needs QA |
| 3 | Date Boundary Controls | ✅ Complete | webapp.json + app.js monkey-patch |
| 3b | Date Boundary Enhancements | ✅ Complete | Inside constraint + validation UI |
| 4 | JS Enhancements | ✅ Complete | adjustHeaderLabels() + applyDateBoundaryPatch() |
| 5 | Version Bump | ✅ Complete | 0.3.0 → 0.4.0 |
| 6 | Frappe clientWidth Patch | ✅ Complete | Null guards in frappe-gantt.umd.js |

---

## 🔴 REMAINING QA ITEMS (For Next Session)

### Must Verify After Reload
- [ ] clientWidth error is gone on view mode transitions
- [ ] Week view >= 50 shows "11 - 17" format (no month names)
- [ ] Week view < 50 shows "11" format (single day)
- [ ] Month view >= 75 shows "January"
- [ ] Month view >= 39 shows "Jan"
- [ ] Month view < 39 shows "J"
- [ ] Year view responsive formatting works
- [ ] Sticky header (Feature 2b) - does it work now?

### Known Bugs (From QA Report - NOT YET FIXED)
| Bug | Severity | Description |
|-----|----------|-------------|
| Data fails to populate | HIGH | Transitions: Hour→Quarter Day, Quarter Day→Half Day, Half Day→Day lose data |
...
### Feature Requests (From QA Report - NOT IMPLEMENTED)
...
| `webapps/gantt-chart/webapp.json` | 🔄 Needs Update | Date boundary params (done), minI change (pending) |
| `webapps/gantt-chart/app.js` | 🔄 Needs Update | Date constraints (done), responsive abbreviations (pending) |
...
Frappe Gantt stores:
- `options.view_mode` → String ("Week", "Month", "Year")
- `config.view_mode` → Object ({ name: "Week", step: 7, ... })
...
- [ ] Header stays fixed during vertical scroll (needs verification)
...
### Responsive Abbreviation Thresholds (VERIFIED)
| View | Threshold | Format |
|------|-----------|--------|
| Week | >= 50 | "11 - 17" (day range, no month) |
| Week | < 50 | "11" (single day) |
| Month | >= 75 | "January" |
| Month | >= 39 | "Jan" |
| Month | < 39 | "J" |
| Year | >= 34 | "2024" |
