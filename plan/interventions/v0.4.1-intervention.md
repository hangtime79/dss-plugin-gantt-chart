...
**Started:** 2025-12-22
**Status:** Ready for QA Retest (SVG Width Fix Applied)
...
| Bug | Status | Notes |
|-----|--------|-------|
| Data fails to populate | ✅ Fixed | maintain_scroll + SVG width patches applied |
| Scroll position jumps | ✅ Fixed | Now scrolls to Today on view change |
| SVG width 100% issue | ✅ Fixed | Patched set_dimensions() to always set pixel width |
| Month/Year Today button | ✅ Fixed | Patched get_closest_date() date parsing |
| Zoom-out auto-center | ✅ Fixed | Changed smooth scroll to instant in set_scroll_position |

---

## 🔴 NEW ISSUE FOUND IN QA (2025-12-22)
...
**QA Observation:**
```
width=100%           → Partial render (BROKEN)
width=20250px        → Full render (WORKS)
```
...
- [ ] Switch Hour → Quarter Day → Half Day → Day: data visible (was broken before)
...
- [ ] Set Fixed Start Date (e.g., 2024-06-01) → chart starts at that date
- [ ] Set Fixed End Date (e.g., 2024-12-31) → chart ends at that date
- [ ] **Switch view modes with boundaries set** → boundaries still respected
- [ ] Clear both dates → chart returns to automatic date calculation
- [ ] Set Start >= End → error message displayed, chart blocked
